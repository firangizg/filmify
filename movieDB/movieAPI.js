...
async function getPages(certification) {
    ...
    * the movies should be available in English, people in the US should be able to watch them, and they should have a rating of above 4 stars */
    const url = `https://api.themoviedb.org/3/discover/movie?certification_country=US&certification=`+ certification +`&include_adult=false&include_video=false&language=en-US&sort_by=popularity.desc&vote_average.gte=4&vote_count.gte=100&watch_region=United%20States&with_runtime.gte=40&page=`;    //the headers and authorization needed to make the request -- for now the API_KEY is in an env file
    const options = {
        method: 'GET',
        headers: {
